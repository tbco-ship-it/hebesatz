(async function () {
  ...
  function card(g) {
    ...
    if (split) return `<section class="sheet quiet"><p class="sheet-label">${where}</p><div class="sheet-num"><span class="num num-txt">Wohnen / Nichtwohnen</span><span class="pct">Grundsteuer B 2025</span></div><p class="sheet-title">${g.name}</p><p class="data-warning">Die Stadt setzt 2025 voraussichtlich getrennte Hebesätze für Wohn- und Nichtwohngrundstücke; die amtliche Tabelle führt keinen Wert (bis 2024: ${pct(g.b24)}). Rechnen Sie auf der Gemeindeseite mit dem Hebesatz aus Ihrem Bescheid.</p><p class="sheet-actions"><a class="next" href="${base}${LA[g.lc][1]}/${g.slug}/">Zur Gemeindeseite mit Rechner</a></p></section>`;
